...
const Profile: React.FC = () => {
  ...
  const [followStatus, setFollowStatus] = useState<
    "Follow" | "Requested" | "Unfollow" | "Accept" | "Reject"
  >("Follow");
      ...
      if (paramsId && url) {
        try {
          const res = await axios.get<User>(
            `${url}/api/user?userId=${paramsId}`
          );
          setProfileUser(res.data);
          const allPostRes = await axios.get(
            `${url}/api/post/profile/${res.data?.username}`
          );
  ...
  const handleFollowAction = async () => {
    ...
    try {
      ...
      if (followStatus === "Follow") {
        ...
        response = await axios.put(`${url}/api/user/${paramsId}/unfollow`, {
          userId: user._id,
        });
        setFollowStatus("Follow");
        setProfileUser((prevUser) =>
        ...
        response = await axios.put(
          `${url}/api/user/${user._id}/accept-request`,
        ...
        response = await axios.put(
          `${url}/api/user/${user._id}/reject-request`,
          { requesterId: paramsId }
        );
        setFollowStatus("Follow");
  ...
  const handleUnblockAction = async () => {
    try {
      if (paramsId && user) {
        await axios.put(`${url}/api/user/${user._id}/unblock`, {
          userId: paramsId,
        });
        setProfileUser((prevUser) =>
          prevUser
            ? {
                ...prevUser,
                blockedByMe: prevUser.blockedByMe.filter(
                  (id) => id !== user._id
                ),
              }
            : null
  ...
  const handleBlock = async () => {
    try {
      if (paramsId && user) {
        await axios.put(`${url}/api/user/${user._id}/block`, {
          userId: paramsId,
        });
        setProfileUser((prevUser) =>
          prevUser
    ...
    <div className="flex w-full h-full flex-col">
          ...
          <div className="flex flex-1 justify-center mt-10">
            ...
            <div className="ml-8 flex flex-col">
              <div className="flex items-center">
                ...
                {paramsId !== user?._id &&
                  paramsId &&
                  !(profileUser?.blockedByMe || []).includes(paramsId) && (
      ...
      <Modal open={openModal} onClose={handleCloseModal}>
        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white w-1/4 rounded-lg shadow-lg">
          <ul
            className="flex flex-col items-center justify-center py-1 font-semibold font-raleway"
